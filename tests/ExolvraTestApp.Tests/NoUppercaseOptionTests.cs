...
public sealed class NoUppercaseOptionTests
{
    [Fact]
    public void Main_WhenNoUppercaseIsPassed_GeneratesPasswordsWithoutUppercaseLetters()
    {
        CliRun run = RunCli("--quiet", "--count", "20", "--length", "32", "--no-uppercase");
        ...
        foreach (string password in passwords)
        {
            ...
            Assert.All(password, static ch => Assert.False(
                char.IsAsciiLetterUpper(ch),
                $"Expected no uppercase letters, but found '{ch}'."));
        }
    }
